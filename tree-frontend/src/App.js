...
function transalteData(treeResponse) {
  ...
  references.forEach((value, key, map) => {
    ...
    visualTreeElems.set(key.toString(), { name: value, attributes: { id: key }, children: [] })
  })
  ...
  leafs.forEach((sum, lastNodeId, map) => {
      visualTreeElems.get(lastNodeId.toString()).children.push({name:sum, children:[]})
  }
  )
...
export default function App() {
  ...
  useEffect(() => {
    const getData = async()=>{fetch(`http://localhost:8080/tree/getTree`)
      .then((response) => {
        if (response.ok) {
